...
function DashboardPage() {
  ...
  const adminProperty = properties.filter((item) => item?.user.id === 1);

  const approvedProperty = properties.filter(
    (item) => item?.status === "approved" && item?.user.id !== 1
  );

  const unapprovedProperty = properties.filter(
    (item) => item?.status === "unapproved" && item?.user.id !== 1
  );

  const pendingProperty = properties.filter(
    (item) => item?.status === "pending" && item?.user.id !== 1
  );
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        >
          <div className="flex justify-between">
            <div>
              <h5 className="leading-none text-3xl font-bold text-gray-900 dark:text-white pb-2">
                {adminProperty.length}
              </h5>
              <p className="text-base font-normal text-gray-500 dark:text-gray-400">
                Total Properties's Admin
              </p>
            </div>
            {/* <div className="flex items-center px-2.5 py-0.5 text-base font-semibold text-green-500 dark:text-green-500 text-center">
              12%
